...
namespace Syschool.Infra.Data.Repository
{
    public class BaseRepository<TEntity> : IBaseRepository<TEntity> where TEntity : class
    {
        ...
        public IEnumerable<TEntity> Select() => _syschoolContext.Set<TEntity>();
        public TEntity Select(Guid id) => _syschoolContext.Set<TEntity>().Find(id);
        public void Update(TEntity entity) => _syschoolContext.Entry(entity).State = EntityState.Modified;
        public void Delete(Guid id) => _syschoolContext.Set<TEntity>().Remove(Select(id));
    }
}
